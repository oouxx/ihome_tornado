...
class AreaInfoHandler(BaseHandler):
    ...
    def get(self):
        ...
        # 从redis中获取的数据默认为b''类型，因此要解码为utf-8
        try:
        ...
        # json.dumps()默认ensure_ascii为True,即不支持中文，因此要设置为False
        json_data = json.dumps(data, ensure_ascii=False)
        # print(json_data)
        try:
            self.redis.setex(
                "area_info",
                constants.REDIS_AREA_INFO_EXPIRES_SECONDES,
                json_data)
        except Exception as e:
            logging.error(e)
...
class MyHousesHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            logging.error(e)
            return self.write(
                {"errcode": RET.DBERR, "errmsg": "get data erro"})
        houses = []
        if ret:
            for l in ret:
                house = {
...
class HouseImageHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not img_name:
            return self.write(
                {"errcode": RET.THIRDERR, "errmsg": "qiniu error"})
        try:
            ...
            logging.error(e)
            return self.write(
                {"errcode": RET.DBERR, "errmsg": "upload failed"})
...
class HouseInfoHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            ...
            try:
                self.db.execute(
                    "delete from ih_house_info where hi_house_id=%s", house_id)
            except Exception as e:
                logging.error(e)
                return self.write(
                    dict(
    ...
    def get(self):
        ...
        # 先从redis缓存中获取信息
        try:
            ret = self.redis.get("house_info_%s" % house_id).decode('utf-8')
        except Exception as e:
            logging.error(e)
            ret = None
        if ret:
            # logging.info("hit redis cache")
            resp = '{"errcode":"0", "errmsg":"OK", "data":%s, "user_id":%s}' % (
                ret, user_id)
            return self.write(resp)
        ...
        data = {
            ...
            "user_avatar": constants.QINIU_URL_PREFIX +
                           ret["up_avatar"] if ret.get("up_avatar") else ""}
        ...
        try:
            self.redis.setex(
                "house_info_%s" %
                house_id,
                constants.REDIS_HOUSE_INFO_EXPIRES_SECONDES,
                json_data)
        except Exception as e:
            logging.error(e)
        resp = '{"errcode":"0", "errmsg":"OK", "data":%s, "user_id":%s}' % (
...
class IndexHandler(BaseHandler):
    ...
    def get(self):
        try:
            ret = self.redis.get("home_page_data").decode('utf-8')
        except Exception as e:
            logging.error(e)
            ret = None
        if ret:
            ...
            try:
                ...
                logging.error(e)
                return self.write(
                    {"errcode": RET.DBERR, "errmsg": "get data error"})
            if not house_ret:
            ...
            for l in house_ret:
                ...
                house = {
                    "house_id": l["hi_house_id"],
                    "title": l["hi_title"],
                    "img_url": constants.QINIU_URL_PREFIX +
                               l["hi_index_image_url"]}
                houses.append(house)
            json_houses = json.dumps(houses, ensure_ascii=False)
            try:
                self.redis.setex(
                    "home_page_data",
        ...
        if ret:
            ...
            try:
                area_ret = self.db.query(
                    "select ai_area_id,ai_name from ih_area_info")
            except Exception as e:
                logging.error(e)
                area_ret = None
            areas = []
                ...
                for area in area_ret:
                    areas.append(
                        dict(
                            ...
                            name=area["ai_name"]))
            json_areas = json.dumps(areas, ensure_ascii=False)
            try:
                self.redis.setex(
                    "area_info",
                    constants.REDIS_AREA_INFO_EXPIRES_SECONDES,
                ...
                logging.error(e)
        resp = '{"errcode":"0", "errmsg":"OK", "houses":%s, "areas":%s}' % (
            json_houses, json_areas)
        self.write(resp)
...
class HouseListHandler(BaseHandler):
    ...
    def get(self):
        ...
        # 数据查询
        # 涉及到表： ih_house_info 房屋的基本信息  ih_user_profile 房东的用户信息 ih_order_info
        # 房屋订单数据
        ...
        if ret:
            for l in ret:
                house = dict(
                    ...
                    avatar=constants.QINIU_URL_PREFIX +
                           l["up_avatar"] if l.get("up_avatar") else "",
                    image_url=constants.QINIU_URL_PREFIX +
                              l["hi_index_image_url"] if l.get("hi_index_image_url") else "")
                data.append(house)
        self.write(
            dict(
                errcode=RET.OK,
...
class HouseListRedisHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            ret = None
        if ret:
            logging.info("hit redis cache: house list")
        ...
        if 1 == page:
            sql += " limit %s" % (constants.HOUSE_LIST_PAGE_CAPACITY *
                                  constants.HOUSE_LIST_PAGE_CACHE_NUM)
        else:
            sql += " limit %s,%s" % ((page - 1) * constants.HOUSE_LIST_PAGE_CAPACITY,
        ...
        while True:
            page_data = data[i *
                             constants.HOUSE_LIST_PAGE_CAPACITY: (i +
                                                                  1) *
                                                                 constants.HOUSE_LIST_PAGE_CAPACITY]
            if not page_data:
                break
        ...
        try:
            ...
            self.redis.expire(
                redis_key, constants.REDIS_HOUSE_LIST_EXPIRES_SECONDS)
        except Exception as e:
            logging.error(e)
